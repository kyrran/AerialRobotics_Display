...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        self.PYB_STEPS_PER_CTRL = int(self.PYB_FREQ / self.CTRL_FREQ) # 240/48
    ...
    def step(self,
        ...
        #### Save, preprocess, and clip the action to the max. RPM #
            ...
            ###################################change the action type-################################3333
            # print(f"action received:{action}")
            # for i in range(self.NUM_DRONES):
            #     current_pos = self.pos[i][:3]
            #     action_diff = action[i][:3]  # Extract the position difference
            #     target_pos = current_pos + action_diff  # Calculate the target position
                
            #     # print(f"Drone {i} target position: {target_pos} = {current_pos} + {action_diff}")
                
            #     action[i][:3] = target_pos  # Update the action to be the new target position

            clipped_action = np.reshape(self._preprocessAction(action), (self.NUM_DRONES, 4))

            # Debugging output for motor commands
            # print(f"Clipped action (RPMs) for all drones: {clipped_action}")
    ...
    def _housekeeping(self):
        ...
        p.resetDebugVisualizerCamera(cameraDistance=2.8, cameraYaw=-10, cameraPitch=-22, cameraTargetPosition=[0, 0, 1.7])
        p.getCameraImage(1920, 1080, renderer=p.ER_BULLET_HARDWARE_OPENGL)
