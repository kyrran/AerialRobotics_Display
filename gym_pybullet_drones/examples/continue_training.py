...
def main(algorithm, timesteps, demo_path, should_show_demo , hyperparams):
    ...
    callback_on_best = StopTrainingOnRewardThreshold(reward_threshold=900,
                                                     verbose=1)
    ...
    agent.learn(timesteps, log_interval=10, progress_bar=True, callback=[eval_callback, TensorboardCallback()], tb_log_name= algorithm + "_training")
...
def parse_arguments():
    ...
    # Choice of algorithm
    parser.add_argument('-algo', '--algorithm', type=str, choices=['SAC', 'SACfD','PPO'], required=True,
